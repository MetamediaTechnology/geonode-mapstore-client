...
// import DisplaySelector from './nearby/DisplaySelector';
...
const loadFeature = function (radius, center, radiusFeature, layerSelected) {
  ...
  return (dispatch) => {
    axios
      .get(layerSelected.url || DEFAULT_API, {
        params: {
          service: "WFS",
      ...
      .then(({ data }) => {
        ...
        try {
          const layerType = layerInfo.properties.find((lt) => {
            return lt.localType === "Point";
          });
...
const setLayerProperties = function (properties) {
  return {
    type: "NEARBY:SET_LAYER_PROPERTIES",
...
const selector = (state) => {
  return {
    ...
    propertiesList: state.nearby.propertiesList,
  ...
  switch (action.type) {
    ...
    case "NEARBY_FEATURE_LOADED": {
      return assign({}, state, {
        ...
        results:
          action.features.length > state.pageSize
            ? action.features.slice(0, state.pageSize)
            : action.features,
        totalPage:
          action.features.length > state.pageSize
            ? Math.floor(action.features.length / state.pageSize)
            : 1,
        currentPage: 1,
      });
    }
    case "NEARBY:SET_LAYER_PROPERTIES": {
      return assign({}, state, {
    ...
    case "NEARBY:CHANGE_PAGE_NUMBER": {
      return assign({}, state, {
        results: state.nearbyLists.slice(
          (action.page - 1) * state.pageSize,
          action.page * state.pageSize
        ),
...
class NearbyDialog extends React.Component {
  ...
  render() {
    ...
    for (const [index, value] of this.props.results.entries()) {
      if (!value?.properties?.isCircle) {
        const obj_key = Object.keys(value.properties);
        const nearbyObj = [];
        obj_key.map((key) => {
          nearbyObj.push(
            <li>
              <b>{key}</b>: {value.properties[key]}
            </li>
          );
        });
        items.push(
            ...
            >
              <BorderLayout header={this.renderHeader()}>
                <div style={{ padding: "10px" }}>
                  ...
                  {/* <label style={{ marginTop: '5px'}}>What are you looking ?</label>
                                        <div>
                                          <DisplaySelector
                                            responses={layerPropertiesName}
                                            index={0}
                                            setIndex={() => {}}
                                          />
                                        </div> */}
                  <div className="btn-toolbar" style={{ marginTop: "10px" }}>
                    <div className="row">
                      ...
                      <div className="col-md-6 text-right">
                        <button
                          onClick={this.onLockCenter}
                          style={{ float: "right" }}
                          className={
                            this.props.centerLocked
                              ? "btn btn-xs active"
                              : "btn btn-xs"
                          }
                        >
                  ...
                  <div className="mapstore-slider with-tooltip">
                    <Slider
                      ...
                      onChange={(value) => {
                        this.onChangeRadius(value[0]);
                      }}
                    />
                  </div>
                  <div className="location-list">
                    <div className="locations">{items}</div>
                    {items.length > 0 ? (
...
const changeLayerEpic = (action$, { getState = () => {} }) =>
  action$
    ...
    .switchMap(({ layer }) => {
      const center = getState().map.present.center;
      const radius = getState().nearby.radius;
      const geometry = circle([center.x, center.y], radius, {
        steps: 100,
        units: "kilometers",
      }).geometry;
      const feature = featureRadius(radius, geometry);
...
const nearbyResultLoadedEpic = (action$, { getState = () => {} }) =>
  action$
    ...
    .switchMap(() => {
      ...
      const layerPropertiesName = [];
      features.map((result) => {
        if (layerPropertiesName.length === 0) {
          Object.keys(result?.properties).map((key, index) => {
            layerPropertiesName.push({
              title: key,
              id: index,
            });
          });
      ...
      return Rx.Observable.from([
        ...
        setLayerProperties(layerPropertiesName),
      ]);
    });
const changePageEpic = (action$, { getState = () => {} }) =>
  action$
    .ofType("NEARBY:CHANGE_PAGE_NUMBER")
    .filter(() => {
      return (getState().controls.nearby || {}).enabled || false;
    })
    .switchMap(() => {
